...
function findKanimDenim(data, studentID) {
  ...
  const kanim = data.find(
    (item) => item.NIM.startsWith(major) && item.NIM.endsWith(unique) && parseInt(item.NIM.slice(3, 5), 10) < year
  );
  ...
  return (
    <main className="flex h-screen flex-col items-center justify-center w-full py-8 bg-black text-white relative">
      ...
      <div className="h-0 grow pr-2 py-4 w-full max-w-[700px] relative">
        ...
        <div className="w-full flex flex-col h-full overflow-y-auto overflow-x-clip gap-2 py-2 px-6">
          {searchTerm && lazyFilter && filteredData.length == 0 && <p>No results found</p>}
          {filteredData.map((item, idx) => (
            <React.Fragment key={item.NIM}>
              <div
              ...
              {result.kanim && idx === selectedIdx && (
                <div className="w-full rounded-lg px-2 py-2 bg-purple-400/50 bg-gradient-to-tr from-white/80 to-white/50 text-black ml-2">
                  <h2 className="text-xs uppercase font-semibold">Kakak NIM - {result.kanim.NIM}</h2>
                  <p className="">{result.kanim["Nama Lengkap"]}</p>
                </div>
              )}
              {result.denim && idx === selectedIdx && (
                <div className="w-full rounded-lg px-2 py-2 bg-purple-400/50 bg-gradient-to-tr from-white/80 to-white/50 text-black ml-2">
                  <h2 className="text-xs uppercase font-semibold">Adik NIM - {result.denim.NIM}</h2>
                  <p className="">{result.denim["Nama Lengkap"]}</p>
                </div>
              )}
            </React.Fragment>
          ))}

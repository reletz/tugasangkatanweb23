  ...
  function findKanimDenim(data, studentID) {
    const { major, year, unique } = parseStudentID(studentID);

    const kanim = data.find(
      (item) =>
        item.NIM.startsWith(major) &&
        item.NIM.endsWith(unique) &&
        parseInt(item.NIM.slice(3, 5), 10) < year
    );

    const denim = data.find(
      (item) =>
        item.NIM.startsWith(major) &&
        item.NIM.endsWith(unique) &&
        parseInt(item.NIM.slice(3, 5), 10) > year
  ...
  export default function Home() {
    ...
    const [result, setResult] = useState({ kanim: null, denim: null });
    ...
    const handleSearchByID = (id) => {
      if (id && data) {
        const { kanim, denim } = findKanimDenim(data, id);
        setResult({ kanim, denim });
      }
    };
      ...
      <main className="flex h-screen flex-col items-center justify-center w-full py-8 bg-black text-white relative">
        ...
        <div className="w-full max-w-[604px] flex justify-between px-4">
          <div className="relative flex flex-col justify-center">
            <h1 className="text-white relative py-10">Ketik NIM untuk mencari Kakak dan Adik NIM kalian!</h1>
        ...
        <div className="h-0 grow pr-2 py-4 w-full max-w-[700px] relative">
          ...
          <div className="w-full flex flex-col h-full overflow-y-auto overflow-x-clip gap-4 py-2 px-6">
            ...
            {filteredData.map((item) => (
              <div
                key={item.NIM}
                className="__card_item hover:scale-105 group duration-150 ease-in flex w-full rounded-lg px-2 py-4 bg-white/50 bg-gradient-to-tr from-white/80 to-white/50 text-black flex-col md:flex-row" onClick={() => handleCardClick(item.NIM, data)}>
          ...
          <div className="fixed bottom-0 w-full max-w-[680px] p-2 flex-col items-center bg-purple-500/95 bg-gradient-to-tr from-white/80 to-red/50 h-fit rounded-lg">
        {result.kanim && (
          <div className="w-full md:w-auto rounded-lg px-2 py-2 bg-purple-400/50 bg-gradient-to-tr from-white/80 to-white/50 text-black m-4">
            <h2 className="text-lg font-bold">Kakak NIM</h2>
            <p>{result.kanim["Nama Lengkap"]} - {result.kanim.NIM}</p>
          </div>
        )}
        {result.denim && (
          <div className="w-full md:w-auto rounded-lg px-2 py-2 bg-purple-400/50 bg-gradient-to-tr from-white/80 to-white/50 text-black m-4">
            <h2 className="text-lg font-bold">Adik NIM</h2>
            <p>{result.denim["Nama Lengkap"]} - {result.denim.NIM}</p>
          </div>
        )}
      </div>
        </div>
